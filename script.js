...
async function fetchBlogs() {
  ...
  try {
    ...
    querySnapshot.forEach((doc, index) => {
      ...
      card.innerHTML = `
        <div class="blog-header" style="margin-bottom:0.5rem;">
        ...
        <div class="blog-content">
          ...
          <button class="read-more-btn" onclick="viewSingleBlog(${index})">Read more...</button>
        </div>
      `;
...
window.viewSingleBlog = function(index) {
  const post = blogCache[index];
  if(!post) return;
  ...
  document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
  document.getElementById('single-blog').classList.add('active');
  window.scrollTo({ top: 0, behavior: 'smooth' });
};
...
async function fetchUpdates() {
  ...
  try {
    ...
    if(actualIssues.length === 0) { container.innerHTML = `<div class="update-placeholder"><p>No updates.</p></div>`; return; }
    container.innerHTML = '';
    actualIssues.forEach((issue) => {
      ...
      const card = document.createElement('div'); card.className = 'update-card reveal-up';
      card.innerHTML = `<div class="update-content"><span class="update-date">${date}</span><h3 class="update-title">${issue.title}</h3></div>`;
